  ...
  return (
    <div className="p-6 max-w-6xl">
      <div className="mb-6 animate-fade-in">
        ...
        <h1 className="text-base font-semibold text-zinc-100 tracking-tight">Alocação de Usuários</h1>
        <p className="text-sm text-zinc-500 mt-0.5">Horas por usuário no mês</p>
      </div>
      ...
      {/* Users table */}
      <div className="bg-zinc-950/50 border border-zinc-800/60 rounded-lg animate-fade-in-delay">
        {loading ? (
          <div className="p-8 text-center"><p className="text-sm text-zinc-500">Carregando...</p></div>
        ) : !data?.users?.length ? (
          <div className="p-8 text-center"><p className="text-sm text-zinc-500">Nenhum usuário encontrado</p></div>
        ) : (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-zinc-800/60">
                  ...
                  <th className="text-right px-4 py-2.5 text-[11px] text-zinc-500 font-medium">Horas ({monthLabel})</th>
                  <th className="text-right px-4 py-2.5 text-[11px] text-zinc-500 font-medium">Custo</th>
                  <th className="text-right px-4 py-2.5 text-[11px] text-zinc-500 font-medium">Registros</th>
                  <th className="text-left px-4 py-2.5 text-[11px] text-zinc-500 font-medium">Tarefas em Andamento</th>
                </tr>
              </thead>
              <tbody>
                {data.users.map((user: any) => {
                  ...
                  return (
                    <tr key={user.id} className="border-b border-zinc-800/30 hover:bg-zinc-900/40 transition-colors">
                      <td className="px-4 py-2.5 text-zinc-300 font-medium">{user.name}</td>
      ...
      {data?.inProgress && data.inProgress.length > 0 && (
        <div className="bg-zinc-950/50 border border-zinc-800/60 rounded-lg p-4 mt-4 animate-fade-in-delay-2">
          <div className="flex items-center gap-2 mb-3">
            <Briefcase className="w-4 h-4 text-zinc-500" />
            <h2 className="text-sm font-medium text-zinc-200">Tarefas em Andamento</h2>
          </div>
          <div className="space-y-2">
            {data.inProgress.map((task: any) => (
              <div key={task.id} className="flex justify-between items-center py-2 px-2 rounded hover:bg-zinc-900/40 transition-colors">
                <div className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                  <span className="text-xs text-zinc-300">{task.title}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-[11px] text-zinc-500">{task.project?.name || '-'}</span>
                  <span className="text-[11px] text-zinc-500">{task.assignedTo ? data.users.find((u: any) => u.id === task.assignedToId)?.name : 'Sem responsável'}</span>
                </div>
              </div>
            ))}
          </div>
