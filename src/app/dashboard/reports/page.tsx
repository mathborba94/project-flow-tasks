  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="mb-6 animate-fade-in">
        <h1 className="text-base font-semibold dark:text-zinc-100 text-zinc-900 tracking-tight">Relatórios</h1>
        <p className="text-sm dark:text-zinc-500 text-zinc-500 mt-0.5">Análise detalhada da sua operação</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {reports.map((report, i) => (
          <Link
            ...
            className={`dark:bg-zinc-950/50 bg-white border dark:border-zinc-800/60 border-zinc-200 rounded-xl p-5 dark:hover:border-zinc-700/60 border-zinc-200 transition-all group animate-fade-in`}
            style={{ animationDelay: `${i * 50}ms` }}
          >
            <div className={`w-10 h-10 ${report.bgColor} rounded-lg flex items-center justify-center mb-3`}>
              <report.icon className={`w-5 h-5 ${report.color}`} />
            </div>
            <h3 className="text-sm font-medium dark:text-zinc-200 text-zinc-800 group-dark:hover:text-zinc-100 text-zinc-900 transition-colors mb-1">
              {report.title}
            </h3>
            <p className="text-xs dark:text-zinc-500 text-zinc-500 leading-relaxed mb-3">
              {report.description}
            </p>
            <div className="flex items-center gap-1 text-xs dark:text-zinc-600 text-zinc-400 group-dark:hover:text-zinc-400 dark:text-zinc-600 text-zinc-400 transition-colors">
              Abrir relatório
