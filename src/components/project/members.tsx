  ...
  if (loading) {
    return (
      <div className="dark:bg-zinc-950/50 bg-white border dark:border-zinc-800/60 border-zinc-200 rounded-lg p-8 text-center">
        <p className="text-sm dark:text-zinc-600 text-zinc-400">Carregando membros...</p>
      </div>
    )
  }
  ...
  return (
    <div className="dark:bg-zinc-950/50 bg-white border dark:border-zinc-800/60 border-zinc-200 rounded-lg">
      <div className="px-4 py-3 border-b dark:border-zinc-800/40 border-zinc-200 flex items-center justify-between">
        <h3 className="text-sm font-medium dark:text-zinc-200 text-zinc-800">Membros</h3>
        <Button
          size="sm"
          variant="ghost"
      ...
      {showAdd && (
        <div className="p-4 border-b dark:border-zinc-800/40 border-zinc-200 space-y-3">
          <div className="relative">
            <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 dark:text-zinc-600 text-zinc-400" />
            <Input
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Buscar usuário..."
              className="pl-7 dark:bg-zinc-900/60 bg-zinc-50 dark:border-zinc-800 border-zinc-300 border-zinc-300 text-sm"
            />
          </div>
          <div className="max-h-48 overflow-y-auto space-y-1">
            {filteredUsers.length > 0 ? (
              filteredUsers.map(user => (
                <div
                  key={user.id}
                  className="flex items-center justify-between px-3 py-2 rounded-md dark:bg-zinc-900/40 bg-zinc-100"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-6 h-6 dark:bg-zinc-800 bg-zinc-100 bg-zinc-100 rounded-full flex items-center justify-center flex-shrink-0">
                      <User className="w-3 h-3 dark:text-zinc-500 text-zinc-500" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-medium dark:text-zinc-200 text-zinc-800 truncate">{user.name}</p>
                      <p className="text-[10px] dark:text-zinc-600 text-zinc-400 truncate">{user.email}</p>
                    </div>
                  </div>
                  <Button
                    size="sm"
              ...
              <p className="text-xs dark:text-zinc-600 text-zinc-400 text-center py-2">
        ...
        {members.length > 0 ? (
          members.map((member) => (
            <div key={member.id} className="px-4 py-3 flex items-center gap-3">
              <div className="w-8 h-8 dark:bg-zinc-800 bg-zinc-100 bg-zinc-100 rounded-full flex items-center justify-center flex-shrink-0">
                <User className="w-4 h-4 dark:text-zinc-500 text-zinc-500" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm dark:text-zinc-200 text-zinc-800 truncate">{member.user.name}</p>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] dark:text-zinc-600 text-zinc-400">{roleLabels[member.user.role]}</span>
                  <span className="text-[10px] dark:text-zinc-600 text-zinc-400">•</span>
                  <span className="text-[10px] dark:text-zinc-500 text-zinc-500">{projectRoleLabels[member.role] || member.role}</span>
                </div>
              </div>
              <button
